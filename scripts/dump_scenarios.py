...
REVIEW_ITEMS: tuple[tuple[str, str], ...] = (
    ...
    (
        ...
        "**결정 J(U6)+결정 M·U6-5(U6-2) 규칙으로 본다**: (a) **결정 M** — 그 턴에 시점을 "
        "가리키는 낱말(오늘·어제·요즘·이번·첫·때·아까·저번·주말·점심·저녁·담주 등)이 "
        "있으면 **문장 성분과 무관하게** relative, 날짜·시각은 absolute, 없으면 none. "
        "앞 턴 상속 없음. 기간 표현(세 시간·3년)만 시점이 아니라 제외한다 — '이번에' 와 "
        "'이번 인사에' 를 다르게 보지 않는다 "
        ...
        "이벤트를 붙이지 않는다 (h) **U6-5** — 행위의 한쪽이 사용자이면(양방향) 활동 "
        "type 을 쓴다: 꾸짖음·불이익 = conflict, 사용자에게 이득이 되는 호의 = favor, "
        "음식 제공 = meal, 함께한 활동 = meeting, 칭찬 = praise, 마찰도 이득도 아닌 "
        "업무 지시·수업·과제 부과 = other. 부작위('말 안 했어'·'그냥 넘어가주셨는데')는 "
        "사건이 아니고, 한 사건을 여러 턴에 나눠 서술하면 성립한 턴 하나에만 붙인다.",
    ),
    (
