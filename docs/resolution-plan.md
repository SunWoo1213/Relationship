...
- 권장: 세 신호를 다음처럼 정의한다.
  ...
  - **CR-001(2026-09-22, P4 파일럿 결과)**: 관측된 신호만 재정규화해 결합한다(D12 — `s_rule` 미측정이면 분모에서 제외, 비율 5:3:2 유지). 2단계 규칙 충돌은 배제가 아니라 감점(D13). 카드 `docs/wiki/decisions/D12`·`D13`, 변경 요청 `docs/wiki/changes/CR-001.md`.
