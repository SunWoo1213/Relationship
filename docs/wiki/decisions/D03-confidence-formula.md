...
상태: **대체됨(→D12, CR-001 2026-09-22)** — 세 신호가 모두 관측될 때의 산식은 D12 에서도 동일하다 | 해결하는 검증: R4 | 원문: `docs/resolution-plan.md` §1 D3, §3.3
...
**갱신 이력**
- 2026-09-22 **대체됨 → D12**(CR-001 A 수용, 사용자). 사유: P4 실 실행에서 `rule_checked == 0`(141 중 100)일 때 `s_rule = 0` 합산으로 확신도 상한이 0.80 에 묶여 게이트 미달(`failure_cases.md` §4). D12 는 관측된 신호만 재정규화하고 가중치 비율(5:3:2)·보정표·trace 규약은 이 카드 그대로 잇는다. P4 기준선(ef18143)은 이 산식의 결과로 보존.
