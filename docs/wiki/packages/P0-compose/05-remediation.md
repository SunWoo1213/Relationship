...
갱신: 2026-09-05 11:27 (verifier 04-review) | 출처: verify-plan → verify-impl(FAIL 0 WARN 0, `evidence/20260905-1127-verify-impl.txt`) | 열림: 0 (필수 0) | 해소: 2 — 구현 단계 대기 항목 모두 04-review 에서 완료 판정

## F-033bb1 · [권고] 보류 1 건 — 결과는 통과가 될 수 없다
상태: 해소 | 발견: 2026-09-03 (verify-plan) | 해소: 계획 단계 2026-09-03 20:11 (1~4) · 구현 단계 2026-09-05 11:27 (verifier 04-review, 5~7 완료)
...
| 5 | (구현 U1) `.env.example` DB 절에 이름 4개 추가 — `.env` 는 건드리지 않는다 | `grep -c "POSTGRES_" .env.example` | `4` (이름 4줄. 주석 문장에는 `POSTGRES_` 문자열을 쓰지 않는다) | 완료 (verifier 2026-09-05 → `4`, `.env.example` 16~19행. `evidence/20260905-1121-review-static.txt`, 커밋 749bb8e) |
| 6 | (구현 U1) `docker-compose.yml` 이 네 변수를 `${VAR:-기본값}`으로만 읽고 기본값이 `.env.example` 예시값과 같음 | `grep -c "POSTGRES_.*:-" docker-compose.yml` | 4 (USER/PASSWORD/DB/PORT 각 1) | 완료 (verifier 2026-09-05 → `5` = environment 8·9·10행 + ports 12행 4개, healthcheck 17행이 USER/DB 를 한 번 더 참조. 기본값 app/pass/relationship/5432 = `.env.example` 예시값. `evidence/20260905-1121-review-static.txt`, 커밋 749bb8e) |
| 7 | (구현 U2·U3) `scripts/db_check.py` 에 `DATABASE_URL`↔`POSTGRES_*` 불일치 경고(이름만) 구현, README 로컬 DB 절에 "두 값을 같게 유지" 안내 | `python scripts/db_check.py` 출력과 `grep -n "DATABASE_URL" README.md` | 불일치 시 변수 이름만 담긴 `WARN` 줄(비밀번호 값 없음), README 로컬 DB 절에 안내 문장 | 완료 (verifier 2026-09-05 실행: `DATABASE_URL=<예시값> POSTGRES_PORT=5433 python scripts/db_check.py` → 1행 `[warn] POSTGRES_PORT 가 DATABASE_URL 과 다르다`(값 없음, exit 코드는 접속 결과로만 결정) `evidence/20260905-1121-review-db-check.txt`; `POSTGRES_*` 부재 시 경고 없음(같은 파일 마지막 블록·`tests/test_db_check.py::test_database_url_used_when_present_without_mismatch`). README 123·142행 안내 `grep -n DATABASE_URL README.md`. 변이 검사로 테스트가 값 노출·비교 누락을 잡음 `evidence/20260905-1121-review-mutation.txt`. 커밋 0ee6e25·7397966) |

### 재검증
- 명령: `bash .claude/scripts/verify-plan.sh P0-compose` (계획 단계. 구현 뒤에는 `verify-impl.sh P0-compose`)
- 결과 파일(evidence/): `evidence/20260903-verify-plan-2-final.txt` (2026-09-03 20:11, FAIL=0 WARN=1 — "보류 0건" PASS. 남은 WARN 은 F-0ffff5 의 README 행) + 단계 판정 명령 출력 `evidence/20260903-remediation-check-2.txt`. 재검증 1회차. 구현 뒤 재검증 2회차: `evidence/20260905-1127-verify-impl.txt` (2026-09-05, FAIL=0 WARN=0) + 단계 5~7 판정 `evidence/20260905-1121-review-static.txt`·`-review-db-check.txt`·`-review-mutation.txt` (04-review §3).
...
상태: 해소 | 발견: 2026-09-03 (verify-plan) | 해소: 계획 단계 2026-09-03 20:11 (1~3) · 구현 단계 2026-09-05 11:27 (verifier 04-review, 4~5 완료. verify-impl 에는 이 WARN 이 없다)
...
| 4 | (구현 U3) `docs/wiki/registry.md` 32행 비고에 "P0-compose: 로컬 DB 절" 추가 — 새 README 행을 만들지 않는다 | `grep -c "| README.md |" docs/wiki/registry.md` | `1` (여전히 한 행) | 완료 (verifier 2026-09-05 → `1`. 32행 비고에 "P0-compose: 로컬 DB 절 추가(2026-09-04), pg16 태그는 P9-infra 에서 재확인" 추가, 소유 `하네스` 유지. `evidence/20260905-1121-review-static.txt`, 커밋 7397966) |
| 5 | (구현 U3) `README.md` 121행 예약 문장을 실제 로컬 DB 절로 교체 | `grep -n "docker compose up -d" README.md` | "로컬에서 해 보기" 절 안에서 1건 이상 | 완료 (verifier 2026-09-05 → 127행 1건, "로컬에서 해 보기" 절 안 "### 로컬 DB (docker-compose + pgvector)" 121~144행. `evidence/20260905-1121-review-static.txt`, 커밋 7397966) |

### 재검증
- 명령: `bash .claude/scripts/verify-plan.sh P0-compose` (계획 단계. 구현 뒤에는 `verify-impl.sh P0-compose`)
- 결과 파일(evidence/): `evidence/20260903-verify-plan-2-final.txt` (2026-09-03 20:11, FAIL=0 WARN=1 — 이 소견의 WARN 이 그 1건. 02-plan-verify §3 에 보류 사유가 아닌 이유를 적었다) + `evidence/20260903-remediation-check-2.txt`. 재검증 1회차. 구현 뒤 재검증 2회차: `evidence/20260905-1127-verify-impl.txt` (FAIL=0 WARN=0) + 단계 4~5 판정 `evidence/20260905-1121-review-static.txt`.
