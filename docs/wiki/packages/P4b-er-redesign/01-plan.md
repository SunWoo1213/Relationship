...
- **U4 이행 해석 확정 (사용자, 2026-09-22).** U4 를 커밋하기 전에 네 가지를 확정했다. (1) **결정 D(i) 해석**: 재계산은 기록된 `weights`·`rule_checked` 로 제품 `combine()` 호출을 그대로 재현하고, `weights_effective` 는 `_check_weights_effective()` 교차 확인에 쓴다. `weights_effective` 를 `combine()` 의 `config` 로 넘기면 `(0.625·s_llm+0.375·s_emb)` 와 제품이 실제로 계산한 `(0.5·s_llm+0.3·s_emb)/0.8` 이 마지막 비트에서 어긋나(무작위 20000 표본 중 7777건, 최대 2.22e-16) `max_abs_diff == 0.0` 기준이 깨진다 — 기준을 낮추는 대신 호출을 재현한다(원칙8, evidence `20260922-1955-u4-recompute-weights-choice.txt`). (2) **`METRICS_SCHEMA_VERSION` 1 → 2 유지**: meta 두 키와 `subsets.penalized_merge` 로 구조가 바뀌었으므로 판 번호도 사실대로 적는다. (3) **`report.py` 부분집합 표 3열 유지**: 결정 H(i) 가 명시한 것은 메타 2줄뿐이지만 그것만으로는 D13 위험 계측이 `metrics.json` 안에만 남아 리포트 독자가 볼 수 없다. (4) **P4 기준선의 명시적 거부 수용**: 옛 trace 는 `weights_effective` 가 없어 `--recheck-traces` 가 1410줄 전건을 거부하고(rc=1), 판 1 `reports/metrics.json` 은 `--validate` 가 거부한다 — 거부 사유는 판 번호가 아니라 결정 H(i) 의 meta 두 키다(evidence `20260922-2020-u4-known-baseline-effects.txt`). 옛 산식으로 계산된 값을 새 산식으로 조용히 통과시키는 것이 더 나쁘다. 기준선을 다시 검증해야 하면 그 시점 코드(`git show f01ea35:scripts/run_pilot_eval.py`)로 한다. 기준선 파일은 읽기만 한다.
