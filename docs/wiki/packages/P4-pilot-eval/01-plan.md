...
- reports/pilot/raw-<ts>.jsonl.gz — 원시 판정(재계산·재현의 입력. 결정 E 이행 방식 = gzip, 사용자 2026-09-21 — pre-commit 5MB 한도, 평문은 커밋하지 않는다)
...
- [eval-agent] **파일럿 평가** (오병합률·미검출률·보정표·곡선 초안) / 의존: P3, P3-llm-providers / 수용기준: `reports/metrics.json`, `reports/calibration.json` 생성. **미달이면 재시도가 아니라 실패 케이스 분석을 산출물로 남기고 ER 설계(resolution-plan 3.3)를 재설계한다**
  ...
  - "**파일럿 평가**" → `data/scenarios/` **실물 40건 전부**에 대해 다섯 방식이 돌았다. 판정: `python -c "import json,gzip,collections;rows=[json.loads(l) for l in gzip.open('reports/pilot/raw-<ts>.jsonl.gz','rt',encoding='utf-8')];print(len({r['scenario_id'] for r in rows}), sorted({r['method'] for r in rows}))"` → `40 ['embedding_only','exact_norm','exact_raw','llm_single','proposed']`. 픽스처·부분 표본이 아니다.
...
로컬 컨테이너는 5433 이므로 포트를 셸 변수로 넘긴다(`.env` 는 읽지 않는다, security.md §1). 아래 `open('reports/….json')` 한 줄 명령은 Windows cp949 로케일에서 `UnicodeDecodeError` 가 나므로 `PYTHONUTF8=1` 을 앞에 둔다(U7 실측 2026-09-22, 명령 본문은 바꾸지 않는다).
...
- **결정 E — `reports/` 산출물 커밋 범위.** (i) **`metrics.json`·`calibration.json`·`curve.csv`·`eval.md`·`failure_cases.md` + 원시 `pilot/raw-<ts>.jsonl` 을 전부 커밋한다**[개정 2026-09-21 사용자: 원시는 결정적 gzip `pilot/raw-<ts>.jsonl.gz` 로 커밋 — 스텁 실측 평문 7,404,682 bytes 가 `.githooks/pre-commit` 5MB 한도 초과, 훅 변경 없음, 1a5643b](**권장** — 원칙8 "평가 수치는 재현 가능해야 한다". 원시 JSONL 이 없으면 지표를 다시 계산할 방법이 LLM 재호출뿐이다. 크기는 40 × mention × 5 × (결정 C-i 이면 1벌) 수준으로 작다) / (ii) 집계본만 커밋하고 원시는 로컬 보관(저장소는 가볍지만 재계산 불가) / (iii) 전부 `.gitignore`(재현성 포기 — 비권장). **어느 쪽이든 프롬프트 원문·키·응답 원문 중 개인정보·비밀은 넣지 않는다**(security §1; 시나리오는 가상 성명이라 본문 자체는 안전).
