...
- **한국어 구어체 부족.** LLM 초안은 문어체·완결 문장으로 기울고, 실제 사용자는 "오늘 김팀장이랑 또 부딪힘ㅋㅋ", "걔 어제 밥 사줬는데" 처럼 쓴다. 완화: 시나리오마다 (a) 조사 생략·종결어미 축약 (b) 오타 1개 이하 (c) 이모지·ㅋㅋ 허용 (d) 발화 길이 8~60자 규칙을 두고 검수 항목에 넣는다. 이 리스크가 현실화되면 P4 수치가 실사용보다 좋게 나온다 — 즉 **과대평가 방향의 편향**이므로 반드시 검수에서 잡는다.
