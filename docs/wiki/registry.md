...
| 모듈 | 백엔드 패키지 골격(DB 접속 설정) | app/config.py | P1-schema | 9397066, d7113e9, 4dfaf33, 09c2bd1, 03e3ce3 | `DATABASE_URL` 우선·없으면 `POSTGRES_*` 조립·`sqlalchemy_url()` — 접속 설정 단일 출처(`scripts/db_check.py`·`scripts/schema_check.py`·`alembic/env.py` 가 여기서 import). 앱·라우터는 P2-tools |
| 모듈 | SQLAlchemy Base + 명명 규칙 | app/db/base.py | P1-schema | d7113e9, 4dfaf33 | `DeclarativeBase` + `MetaData(naming_convention=...)`(pk/fk/ix/uq/ck) — 제약 이름이 alembic autogenerate·schema_check 기대값의 근거 |
