...
갱신: 2026-09-23 12:40 (U7 완료) — **P4b U0~U7 완료. 남은 것은 04-review 하나뿐이다.** 이번 세션: 재개 → 미커밋 후행 기록 커밋 `9169a04` → Docker 기동 확인 → **U7 을 eval-agent 에 1회 위임**(L-004 승인, `--stage eval-agent`) → 판정 표 21행 전부 실행 **미충족 0 · 부분 1**, evidence `20260923-1214-u7-accept-1.txt`(1~11행)·`-2.txt`(12~21행). registry 기존 행 20개 비고 확장 + 새 행 4개, README 에 P4b 재실행 문단 1개, 카드 10 은 대조만 하고 무수정. 코드·데이터 변경 0.
**메인 세션이 직접 다시 확인한 것**: 게이트 `0.8 [] True True` · 20행 `KeyError` 재현 · 정정 경로 값 · `git diff --name-only -- app/ data/ evaluation/ scripts/ tests/ reports/` 0줄 · 인용 커밋 해시 7개 실재.
**04-review 가 반드시 읽을 것(U6·U7 이 찾아낸 것)**: (1) **판정 표 20행은 경로 오기다** — 01-plan 111행 `m['proposed']['subsets']` 는 `KeyError`, 실제는 `m['proposed']['by_t_merge']["0.8"]['subsets']`. 산출물 결함이 아니다. (2) 오병합·미검출이 사라진 자리는 정답이 아니라 **되묻기**다 — `sc-015` t0 '부장님'·`sc-007` t2 '문실장님' 둘 다 `deferred`. 수용 기준(오병합·미검출 아님)은 충족이지만 '해결'이 아니다. (3) **보수 강등 5건은 전부 귀속이 골드였다** — 이 정책이 막은 오병합 0건, 대신 정답 5건을 되물었다(마찰 31건의 16%). (4) **실행 간 `s_llm` 자기보고가 136 mention 중 60건 달랐다** — 게이트 개선을 D12·D13 단독 효과로 읽으면 안 된다. 다만 `exact_raw`·`exact_norm`·`embedding_only` 는 결정 변화 0 이라 지배가 풀린 것이 베이스라인 악화 때문은 아니다. (5) **`penalized_merge` 정의가 둘이다** — `metrics.json` 이 세는 '귀속 인물 자신이 감점 후보' 기준 **14건**, 원시에서 '후보 아무나 감점' 기준 22건. 둘 다 오병합 0. 어느 정의인지 함께 적어야 한다. (6) **9행 실 실행 evidence 에 rc 줄이 없다** — 더 강한 증거가 필요해도 **재실행하지 말고** `20260923-0021-u5-postrun-verify.txt` 를 근거로 삼는다(원칙8). **게이트: `0.8 [] True True`** — T_merge 0.8 에서 오병합 0/132·미검출 0/132, 베이스라인 4종 모두 `dominates_proposed=false`, F1 0.8972(precision 1.0·recall 0.8136), 되묻기 31건 23.5%, 비용 $0.0274(상한 $5). P4 기준선은 `0.8 ['embedding_only'] True False` 로 미달이었다. **커밋 메시지는 사람이 읽는 문장 형식(사용자 지시) 계속 적용.**
active: **P4b-er-redesign** | frozen: none | 브랜치: dev — **로컬이 origin/dev(4338eea) 보다 앞선다**: `9169a04`(후행 기록) + U7 커밋. 푸시는 04-review 까지 마친 뒤 한 번에 한다(L-003 마커가 생기면 다음 단계를 못 시작하기 때문). **origin/main = `0dbf2c9` — dev 와 갈라져 fast-forward 승격 불가**(열린 질문 참조). Docker `capstone2-postgres-1` 5433. 미추적 2파일은 **의도적으로 커밋하지 않는 것**: `reports/pilot/raw-20260922-150931.jsonl`(평문 7.7MB, 훅 5MB 한도 — `.gz` 만 커밋)·`reports/pilot/metrics-stage.json`(중간 산출물, P4 도 커밋하지 않았다). 지우지 말고 그대로 둔다.
...
## 바로 다음에 할 것 (순서대로)
1. ~~U7~~ **완료**(위 갱신 줄). 판정 표 21행 미충족 0·부분 1.
