...
갱신: 2026-09-10 14:50 (U4 임베딩 단독 완료 — eval-agent 산출물·evidence 확인, /commit 승인 대기)
active: **P3-baselines** | frozen: none | 브랜치: dev = **bcb2fad**(U3), origin dev = main = 0e3447a(미푸시 3, U4 커밋 후 4). **U1 05d90f0 · U2 f212ad6 · U3 bcb2fad 완료. U4 완료(커밋 대기)**: `evaluation/resolvers/embedding_only.py`·`resolvers/__init__.py` import·`tests/test_baseline_embedding_only.py` 53건·`test_baseline_exact_match.py` 순서 단언 `[:3]`, evidence `20260910-1437-u4-{pytest(173, skip 0),isolation,pytest-all(640)}.txt`, `app/` 0줄. eval-agent 판단 8건(동점 강등은 merge 밴드만, embedding_skipped 는 band_for 에 위임) → 03-log 14:50 항목 → verifier 04-review. **다음**: /commit(사용자 승인) → **U5 LLM 단일 프롬프트**(`llm_single`, 01-plan U5 행·결정 E(ii)·I·J, R-3·R-4·R-5 — `app/er/judge.py` 오류 매핑 공개 승격 1건이 `app/` 유일 예외, `tests/test_er_judge.py` 무수정 통과 조건) eval-agent(L-004) → U6(R-6·R-7) → U7(R-1) → U8 → verifier. DB 5433 UP. 결정 A~J = 01-plan "결정 확정" 2블록, 권고 R-1~R-9 = 02-plan-verify §3.
