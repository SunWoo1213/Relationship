...
## 바로 다음에 할 것 (순서대로)
1. **U8 커밋 — 승인 완료(이 커밋에 포함)**: `reports/failure_cases.md` + evidence + registry + 03-log(U7 `pending`→ef18143 치환됨; U8 항목 `pending` 은 다음 커밋에서) + HANDOFF·journal. **add 금지**: `reports/pilot/` 의 평문 `raw-….jsonl`·`metrics-stage.json`·pilot 사본(`metrics.json`·`calibration.json`·`curve.csv`·`eval.md` — reports/ 에 이미 커밋됨). `.gitignore` 추가 여부는 사용자 결정.
2. 사용자가 `reports/failure_cases.md` §4·§8 을 읽고 **`/devlog change` 후보 3 중 무엇을 CR 로 올릴지 결정**(AskUserQuestion). CR 이면 `CURRENT.md frozen`, S3.3 카드·D3 가중치 영향 분석은 태그로.
3. U9: 카드 10·01-plan 판정 명령 encoding 문구 수정(docs) → 러너 `safe_summary` FIX 후보(사용자 결정) → registry `pending` 해시 확정 → `/devlog done`(verifier 04-review, 결과 "부분완료 + 실패 케이스 분석" 원칙8, verifier 가 `--recheck-traces`·`.gz` 재계산 재확인). **P5 이후 시작 금지**(결정 K 개정 (a)).
4. 보류: 09 카드 §2 결정 10개(P9 전), anthropic·gemini 스모크(선택), P0-cost(흡수됨 — cost_estimate §3·§4 실측 완료), F-46f1eb·F-036185, 하네스 L-nnn(verify-impl 6번), dev 푸시(13 미푸시 — 푸시 뒤 L-003).
