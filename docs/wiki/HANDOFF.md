...
갱신: 2026-09-07 00:05 (U4 완료·커밋 — 40건 완성, U5 위임 대기)
active: **P1-pilot-dataset** | frozen: none | 브랜치: dev (origin/dev 09875aa, main bdf9f70 승격 보류; 로컬 U3 aa6ecfc + U4 커밋 미푸시). **데이터 40건 완성**(promotion 8·alias 8·pronoun 8·normal 10·new_person 6, trap 12, ambiguous 3, virtual_names 108, --strict rc=0). **다음: U5**(eval-agent, L-004 승인 후) — 사용자 결정: **schema_version 2**: 선택 필드 `passing_mentions[{turn,surface,why}]` + 검사 14(surface 존재)·15(정상 mention 과 미중복) + 검사 11(파일명↔category), sc-038~040 에 필드 채움(기존 데이터 무변경); manifest.distribution 배분표(관계 태그·위계·하위 유형·trap 열·schedule_utterance_scenarios[sc-025,027,029,033]); 01-plan 에 턴 수 2~6 규칙 한 줄; 검수 패킷(verifier 용, virtual_names 108 실존 여부 눈검수 항목 포함); 수용 기준 evidence(--strict rc=0·건수·pytest). 이후 verifier 라벨 검수(새 컨텍스트, evidence/<ts>-label-review.md) → U6 → U7.
