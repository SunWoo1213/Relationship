...
갱신: 2026-09-05 20:10 (U5 구현 완료, 커밋 직전)
active: **P2-tools** | frozen: none | 브랜치: dev (dev = origin/dev = **main = 5dc95bb**). 미푸시 로컬 커밋: 7b793ba(착수)·f217190(U1)·4eca3e9(U2)·a9cb254(U3)·f318d58(U4). 미커밋: U5 산출물(records.py, tests 26, persons _owned_person 추출, EventOut.created_at, evidence 4, 위키) — U5 커밋으로 정리 중
...
7. **U5 구현 완료(backend-agent, 2026-09-05)**: app/tools/records.py add_event(ctx, person_id, type, content, occurred_at, raw_utterance)·add_schedule(ctx, person_id, title, scheduled_at) — EVENT_TYPES 검사, raw_utterance 원문 저장, `_require_aware`(naive → InvalidValue, **UTC 정규화 저장·반환** — trace 가독성·P6 90일 윈도 기준 통일), `_owned_person` 은 persons.py 에서 추출해 공유, 패턴 감지·승격 없음(같은 type 3건 넣어도 person_facts 0행 테스트). EventOut.created_at 추가. 테스트 26건, 변이(naive 허용) 2건 FAILED 후 원복. pytest 131. evidence 20260905-1633-*. → `/commit` 진행 중.
8. U6(사용자 승인 → `--stage backend-agent`): app/tools/questions.py — ask_user(ctx, kind, question, options, context) -> PendingQuestionOut{question_id, status:"pending"}: kind∈QUESTION_KINDS, options 비어 있지 않음, **context 에 AFFIRMATIVE_KEY(affirmative_options ⊆ options) 필수(identity/new_person)** — U4 규약, session_id=ctx.session_id, 동기 대기 없음(D2); answer_question(ctx, question_id, answer): 없음 QuestionNotFound, 이미 answered/만료 QuestionNotAnswerable(409), options 밖 InvalidValue(422), answered_at=ctx.now(); question_status(q, now): answered / expired(created_at < now-24h, S3.4) / pending. tests/test_tools_questions.py(dbtest, now 주입으로 만료 테스트). 이후 U7 → U8 → U9.
...
## 열린 질문 · 사용자 결정 대기
- U5 커밋 승인 → U6 시작 승인(L-004). P1-pilot-dataset·P0-cost 착수 시점.
