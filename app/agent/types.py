...
# ---------------------------------------------------------------------------
# M-2(i) -- schedule 재개 질문의 "모르겠어요" 옵션 리터럴 (U5 가 정한다)
# ---------------------------------------------------------------------------

#: `ask_user(kind="schedule")` 의 마지막 옵션(01-plan 결정 K(i)·M-2(i)).
#: 이 값으로 답하면 `add_schedule` 을 부르지 않는다 -- U5 기록 단계(질문을
#: 만드는 쪽)와 향후 U7 재개(답을 읽는 쪽)가 같은 문자열을 리터럴로
#: 중복해서 쓰지 않도록 여기 하나만 둔다(단일 출처).
SCHEDULE_UNKNOWN_OPTION = "모르겠어요"
