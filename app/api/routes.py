"""Refs: P2-tools U8 R7 D2 S3.4 -- FastAPI 라우트(`GET /health`,
...
그 자리에서 `None` 으로 흡수하는 것과, `POST /chat` 이 루프 예외를 결정
G·H(01-plan U6)에 따라 직접 삼키는 것뿐이다 -- 둘 다 "판정"이 아니라
정해진 규약대로 흡수하는 것이다.
...
from app.agent import LoopError, Proposer, run_turn
...
from app.api.schemas import AnswerIn, AnswerOut, ChatIn, ChatOut, HealthOut, StoredOut
